document.addEventListener('DOMContentLoaded', () => {
    ...
    // Removed the single patientSearchInput (main search box) functionality here
    const tableHeaders = document.querySelectorAll('#patientTable th.sortable');
    const columnFilters = document.querySelectorAll('.column-filter'); // Select all new filter inputs
    
    let editingPatientId = null; 
    let currentSortKey = 'id';
    let sortDirection = 'asc';

    // --- Utility and Validation Functions (Same as before) ---
    ...
    // --- Setup and Data Handling (Same as before) ---
    ...
    // New: Add event listeners to all column filter inputs
    columnFilters.forEach(filterInput => {
        filterInput.addEventListener('keyup', renderPatients); // Re-render table on every key press
    });
    ...
    function filterAndSortPatients(patientsArray) {
        let filtered = [...patientsArray]; // Start with all patients

        // Loop through all filter inputs and apply filters sequentially
        columnFilters.forEach(filterInput => {
            const filterValue = filterInput.value.toLowerCase().trim();
            if (filterValue) {
                // Get the data key from the input's ID (e.g., 'filter-firstname' -> 'firstname')
                const filterKey = filterInput.id.replace('filter-', '');

                filtered = filtered.filter(patient => {
                    // Check if the patient data for that key includes the filter value
                    const patientValue = String(patient[filterKey]).toLowerCase();
                    return patientValue.includes(filterValue);
                });
            }
        });

        // 2. Sort the filtered array (same logic as before)
        filtered.sort((a, b) => {
    ...
    // --- Edit and Delete Functionality (Global access via window) ---
    ...
    window.deletePatient = function(patientId) { /* ... same as before, with alert message ... */
    ...
    function renderPatients() {
        ...
        patientsToDisplay.forEach(patient => {
            ...
            row.innerHTML = `
                <td>${patient.id}</td><td>${patient.firstname}</td><td>${patient.lastname}</td>
                <td>${patient.age}</td><td>${patient.DOB}</td><td>${patient.gender}</td>
                <td>${patient.height}cm</td><td>${patient.weight}kg</td><td>${patient.contact}</td>
                <td>${patient.email}</td><td>${bmiDisplay}</td>
                <td><button onclick="editPatient('${patient.id}')">Edit</button></td>
                <td><button onclick="deletePatient('${patient.id}')" class="delete-btn">Delete</button></td>
            `;
    ...
    // --- Main Form Submission Handler (Same as before) ---
    form.addEventListener('submit', (e) => {
        ...
        if (allValid) {
            ...
            localStorage.setItem('patients', JSON.stringify(patients)); renderPatients(); form.reset(); displaySuccessMessage();
        }
    });
