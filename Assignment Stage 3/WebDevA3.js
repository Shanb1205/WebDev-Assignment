document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateInput(inputElement) {
        ...
        if (!inputElement.validity.valid) {
            displayError(inputElement.id, inputElement.validationMessage);
            return false;
        }
        return true;
    }
    ...
    // Attach event listeners for immediate feedback
    const inputs = form.querySelectorAll('input, select');
    inputs.forEach(input => {
        input.addEventListener('input', () => {
            if (input.id === 'lastname') { validateLastNameCustom(); } 
            else if (input.id === 'DOB') { validateDOB(); }
            else if (input.value.trim() !== '') { validateInput(input); }
        });
    ...
    function generatePatientId() {
        let nextId = parseInt(localStorage.getItem('nextPatientId')) || 1000;
        const newId = nextId;
        localStorage.setItem('nextPatientId', nextId + 1);
        return 'P' + newId;
    }
    ...
    function highlightBmiCategory(bmi) {
        document.querySelectorAll('#bmiLegendTable tr').forEach(row => {
    ...
    function filterPatientsTable() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            for (let j = 0; j < columnsToSearch.length; j++) {
                const cell = cells[columnsToSearch[j]];
                if (cell) {
    ...
    if (patientSearchInput) {
        patientSearchInput.addEventListener('keyup', filterPatientsTable);
    }
    ...
    window.editPatient = function(patientId) {
        ...
        if (patientToEdit) {
            document.getElementById('firstname').value = patientToEdit.firstname;
            document.getElementById('lastname').value = patientToEdit.lastname;
            document.getElementById('age').value = patientToEdit.age;
            document.getElementById('DOB').value = patientToEdit.DOB;
            document.getElementById('gender').value = patientToEdit.gender;
            document.getElementById('height').value = patientToEdit.height;
            document.getElementById('weight').value = patientToEdit.weight;
            document.getElementById('contact').value = patientToEdit.contact;
            document.getElementById('email').value = patientToEdit.email;
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (allValid) {
            ...
            if (editingPatientId) {
                ...
                document.querySelector('button[type="submit"]').textContent = 'Register Patient';
            } else {
                formData.id = generatePatientId();
